...
def init_db():
    ...
    cur = conn.cursor()
    # 注意：增加了 mode 字段以区分模式 A 和模式 B
    cur.execute("CREATE TABLE IF NOT EXISTS projects (project_id TEXT PRIMARY KEY, created_at TEXT, mode TEXT, total_rent REAL, roommate_count INTEGER)")
...
TEXT = {
    "EN": {
        ...
        "mode_select": "Select Pricing Mode",
        "mode_a": "Mode A: Total Budget Split (We only have a total rent, algorithms decide each room's price)",
        "mode_b": "Mode B: Fixed Price Adjustments (Rooms have fixed lease prices, we need side payments)",
        ...
        "input_desc": "Just tell us how much you'd personally pay for each room's physical features. The total must equal the apartment's total rent.",
        ...
        # 防反悔提示
        "regret_tip": "💡 Note on Pricing: The algorithm ensures everyone secures the room they effectively value most relative to others. Bids are now locked to ensure theoretical fairness and prevent strategic manipulation."
    },
    "ZH": {
        "title": "FairShare 科学分房",
        "subtitle": "为合租室友提供基于算法的选房建议与公平租金分摊。",
        "step1_title": "第一步：定制公寓信息",
        "mode_select": "选择租金模式",
        "mode_a": "模式 A：总价动态分摊 (适用于还没签合同，仅有一口价总预算)",
        "mode_b": "模式 B：固定价格补偿 (适用于房东已定好每间房租，需私下平衡差价)",
        "num_agents": "合租人数",
        ...
        "input_desc": "仅凭你的主观感觉：为了住进这个房间，你最多愿意承担多少房租？(总和必须等于公寓总租金)",
        "agent_name": "你的称呼:",
        "val_for": "对该房的心理估值：",
        "btn_submit": "确认提交",
        ...
        # 防反悔提示
        "regret_tip": "💡 分配说明：算法确保每个人都优先获得了自己相对最看重的房间。为维护机制的防操纵性(Strategy-proofness)，分配结果现已锁定。"
    }
}
...
def create_project(project_id, mode, total_rent, roommate_count, rooms_data):
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    cur.execute("INSERT INTO projects VALUES (?, ?, ?, ?, ?)", (project_id, datetime.now().isoformat(), mode, total_rent, roommate_count))
    for r in rooms_data:
...
def get_project_info(project_id):
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    cur.execute("SELECT mode, total_rent, roommate_count FROM projects WHERE project_id=?", (project_id,))
    proj = cur.fetchone()
    if not proj: return None, None, None, []
...
if not project_id:
    ...
    with st.container(border=True):
        ...
        # 模式选择
        mode = st.radio(t["mode_select"], ["Mode A", "Mode B"], format_func=lambda x: t["mode_a"] if x == "Mode A" else t["mode_b"])
        ...
        if mode == "Mode A":
            total_rent_input = st.number_input(t["total_rent_input"], value=3000.0, step=100.0)
            total_rent_calculated = total_rent_input
        else:
...
# --- 场景 2：数据收集与展示 ---
    ...
    # --- 状态 B：展示分配结果 (锁定状态，不可撤回) ---
        ...
        try:
            assignment, theoretical_prices, fixed_prices, side_payments = compute_envy_free_allocation(users, rooms_data, valuations, total_rent)
            
            # 模式 A：直接显示算法分配的房间和最终租金（极其简洁）
            if mode == "Mode A":
                st.markdown(f"<p style='color:#86868b; font-size:14px;'>Based on everyone's true valuations, here is the mathematically fair allocation and price split.</p>", unsafe_allow_html=True)
                for u in users:
                    ...
                    <div style="padding: 20px; border-radius: 16px; background-color: #fbfbfd; margin-bottom: 20px; border: 1px solid #d2d2d7;">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            ...
                            <div style="text-align: right;">
                                <div style="font-size: 14px; color: #86868b;">{t['rent_to_pay']}</div>
                                <div style="font-size: 20px; font-weight: 600; color: #0071e3;">${fair_p}</div>
                            </div>
                        </div>
                    </div>
